    ...
    document.addEventListener('submit', (e) => {
        if(e.target.id === 'form-add-fuel') {
            ...
            const data = {
                ...
                date: document.getElementById('service-id').value ? (DB.get(DB.KEYS.SERVICE).find(s=>s.id===id)?.date || new Date().toISOString().split('T')[0]) : new Date().toISOString().split('T')[0],
                type: 'Service'
            };
            ...
            e.preventDefault();
            // Save settings
            const userNameVal = document.getElementById('ep-username').value;
            DB.set(DB.KEYS.SETTINGS, { userName: userNameVal });
